...
Rather than describing the system by a wave function, the Lindblad equation governs the evolution of the density operator $t\mapsto \rho_t\in \scrD(\calH)$, which encodes both classical and quantum uncertainty. The dynamics are given by
\begin{align}\label{eq:lindblad}\tag{\textsf{GKSL}}
	\frac{\d}{\d t}\rho_t = -i[H,\rho_t] - \tfrac{1}{2}\sum\nolimits_j [L_j^\dagger,[L_j,\rho_t]],\qquad \rho_0\in\scrD(\calH),
\end{align}
where $H\in\scrO(\calH)$ is a given system Hamiltonian, and the operators $L_j\in \scrB(\calH)$, called \emph{Lindblad operators} or \emph{jump operators}, describe the coupling to the environment. The first term represents coherent unitary evolution, while the second term captures irreversible processes such as decoherence, relaxation, and dissipation.
...
Given a Hamiltonian $H\in\scrO(\calH)$ and a real-valued smooth process $\upalpha_t$, the Schr\"odinger equation driven by the time-dependent Hamiltonian $\dot\upalpha_t H\in \scrO(\calH)$ may be expressed as an evolution in the space of unitaries $\scrU(\calH)$:
\[
	\d U_t = iHU_t\dd \upalpha_t,\qquad U_0 = \sfI_\calH.
\]
In this simple case, the solution may be explicitly expressed as
\[
	U_t = \exp\bigl(i(\upalpha_t-\upalpha_0) H\bigr)\sfI_\calH.
\]
...
In the presence of noise, however, $\upalpha_t$ may no longer be smooth. Nevertheless, if $\upalpha_t = \upomega_t$ is the Brownian motion, then the It\^o formula applies and we get for $U_t\coloneqq \exp(i\upomega_t H)\sfI_\calH$,
\[
	\d U_t = \bigl(iH\dd \upomega_t - \tfrac{1}{2}H^2\dd t\bigr)U_t = iHU_t\,{\circ} \dd \upomega_t,
\]
where $\circ\,\d \upomega_t$ denotes the Stratonovich integral. This is precisely a stochastic Schr\"odinger equation with one noise operator and one driving noise. 

\subsection{Unitary-valued processes}

More generally, we consider a set $\scrZ = \{L_1,\dots,L_r\} \subset \scrO(\calH)$ of orthonormal observables under the Hilbert-Schmidt scalar product on $\scrO(\calH)$, where $1\le r\le n^2$. These observables will often be called noise operators. Further, let $\upomega_t^1,\dots,\upomega_t^r$ be independent standard real-valued Brownian motions and $\upalpha_t^1,\ldots,\upalpha_t^r$ be It\^o processes of the form
\[
	\upalpha_t^j = \upalpha_0^j + \ttb_t^j + \sqrt{\gamma_j}\,\upomega_t^j,\qquad j=1,\ldots,r,
\]
...
\]
The matrix-valued quadratic variation of $\upchi$ is given by
\[
...
\]
Clearly, there are situations where $\scrA_\scrZ=\scrA(\calH)$ for $r<n^2$, in which case, the full group of unitaries is explored, i.e., $\exp(\scrA_\scrZ)= \scrU(\calH)$.

In It\^o form, the equivalent SDE reads
\[
	\d U_t = \bigl( \d \upchi_t + \frl\dd t\bigr)U_t,\qquad \frl := -\tfrac{1}{2}\sum\nolimits_j \gamma_j L_j^2,
\]
where $\frl$ is the Laplace-Beltrami operator associated with the right-invariant connection.

\begin{remark}\label{rem:explicit-sse}
In the case $r=1$, we find, as in the deterministic case, the explicit solution
\[
	U_t = \exp\bigl(i(\ttb_t + \sqrt{\gamma}\upomega_t) H\bigr)\sfI_\calH.
\]
In particular, $U_t$ commutes with $H$ for all $t\ge 0$.
\end{remark}

\paragraph{Towards the Lindblad equation}
To obtain the Lindblad equation \eqref{eq:lindblad} from the stochastic Schr\"odinger equation \eqref{eq:unitary-sse}, we consider noise profiles $\upalpha_t^j = t + \upomega_t^j$, $j=1,\ldots,r$.
...
\begin{align*}
	...
	&= (\d \upchi_t + \frl\dd t)\, \oba_t + \oba_t (\d \upchi_t^\dagger + \frl^\dagger\d t) + \d \upchi_t\,\oba_t \dd \upchi_t^\dagger \\
	&= i \sum\nolimits_j [L_j,\oba_t]\bigl(\d t + \sqrt{\gamma_j}\d \upomega_t^j\bigr) + \tfrac{1}{2}\sum\nolimits_j \gamma_j \bigl(2L_j\,\oba_t\, L_j - L_j^2\oba_t - \oba_t L_j^2\bigr)\dd t \\
	&= -i[H,\oba_t]\dd t  - \tfrac{1}{2}\sum\nolimits_j \gamma_j[L_j,[L_j,\oba_t]]\dd t + i \sqrt{\gamma_j}\sum\nolimits_j [L_j,\oba_t] \dd \upomega_t^j,
\end{align*}
where we set $H\coloneqq -\sum\nolimits_j L_j$. Hence, taking the expectation, we obtain for the density operator $\rho_t\coloneqq \bbE[\oba_t]\in\scrD(\calH)$ the deterministic evolution
\[
	\d \rho_t = -i[H,\oba_t]\dd t  - \tfrac{1}{2}\sum\nolimits_j \gamma_j[L_j,[L_j,\oba_t]]\dd t,
\]
...
In quantum computing, one is often interested in the fidelity of a quantum gate, i.e., a unitary operation. Say, we would like to implement a Hadamard gate on a single qubit on the Hilbert space $\calH=\bbC^2=\text{span}\{\e_0,\e_1\}$, where the Hadamard gate is given by
\[
	U_\sfh = \frac{|\e_0\rangle + |\e_1\rangle}{\sqrt{2}} \langle\e_0| + \frac{|\e_0\rangle - |\e_1\rangle}{\sqrt{2}} \langle \e_1| = \frac{1}{\sqrt{2}}\begin{pmatrix}
		1 & 1 \\ 1 & -1
	\end{pmatrix},
\]
...
\begin{align*}
	U_\sfh &= (1)|\e_+\rangle\langle \e_+| + (-1)|\e_-\rangle\langle \e_-|
	= e^0|\e_+\rangle\langle \e_+| + e^{-i\pi}|\e_-\rangle\langle \e_-| \\
	&= \exp(-i\pi |\e_-\rangle\langle \e_-|) = e^{-i\pi H_\sfh},\qquad H_\sfh\coloneqq |\e_-\rangle\langle \e_-| = \tfrac{1}{2}(\sfI_\calH - \sigma_\mathsf{x}).
\end{align*}
...
\[
	\d U_t = iU_t H_\sfh\dd t,\qquad U_0 = \sfI_\calH.
\]
Notice that $H_\sfh$ is a rank-1 projection on the vector $\e_-$.

In the presence of noise, however, we instead have
\[
	\d U_t^\upalpha = iU_t^\upalpha H_\sfh\circ \d\upalpha_t,\qquad U_0=\sfI_\calH,
\]
where in this example, we consider for simplicity the noise of the form $\upalpha_t = t + \sqrt{\gamma}\upomega_t$. From Remark~\ref{rem:explicit-sse}, we obtain the explicit form $U_t^\upalpha = \exp(i\upalpha_t H_\sfh)$.

A common quantity of interest is the deviation of $U_t^\upalpha\psi$ from $U_t\psi$ for a given unit vector $\psi\in\calH$ in the sense of fidelity, i.e.,
\[
...
\[
	U_t^\dagger U_t^\upalpha = \exp(i\sqrt{\gamma}\upomega_t H_\sfh) = |\e_+\rangle\langle \e_+| + e^{i\sqrt{\gamma}\upomega_t}|\e_-\rangle\langle \e_-|.
\]
...
%	&= |\langle \psi,\psi\rangle|^2 + 2\bigl(\cos(\sqrt{\gamma}\omega_t)-1\bigr)|\psi_+|^2|\psi_-|^2 \\
	= 1 + 2\bigl(\cos(\sqrt{\gamma}\upomega_t)-1\bigr)|\psi_+|^2|\psi_-|^2.
\end{align*}
