...
\[
	\rho_t = \tr\nolimits_{\calH_\text{env}} \bigl[U_t (\rho_0 {\otimes} \rho_{\text{env}}) U_t^\dagger\bigr]\;\in\scrD(\calH_{\text{sys}}),
\]
...
\begin{exercise}
	...
	\[
		[\sfA_j(t),[\sfA_j(t),\sfA_j^\dagger(t)]]=[\sfA_j^\dagger(t),[\sfA_j(t),\sfA_j^\dagger(t)]] = 0\qquad\text{for all $t\ge 0$},
	\]
...
\]
with the following It\^o table for $\d\langle\sfM\sfN\rangle$:
%\begin{figure}[h]\centering\renewcommand{\arraystretch}{1.2}
\begin{center}\renewcommand{\arraystretch}{1.2}
	\begin{tabular}{c|cc} 
	$\d\sfM\backslash\d\sfN$ & $\d \sfA$ & $\d \sfA^\dagger$ \\ \hline 
	$\d \sfA$ & $0$ & $\d\uplambda$ \\
	$\d \sfA^\dagger$ & $0$ & $0$ \\
\end{tabular}
\end{center}
allowing us to further obtain an It\^o formula: For smooth functions $f\colon\bbC\to\bbC$,
\begin{align*}
	\d f(\sfX(t)) = Df(\sfX(t))\dd\sfX(t) + \tfrac{1}{2}D^2f(\sfX(t))\dd\langle\sfX^2\rangle(t).
\end{align*}
...
\[
	\sfX(t) = -itH{\otimes}\sfI_{\calH_\text{env}} + L{\otimes} \sfA^\dagger(t) - L^\dagger{\otimes}\sfA(t)\;\;\in\scrA(\calH_\text{tot}).
\]
...
\begin{align}\label{eq:qsde}\tag{\textsf{QSDE}}
	\d U_t = \bigl( \d\sfX(t) + \frI \dd t \bigr) U_t, \qquad \frI = -\tfrac{1}{2}(L^\dagger L){\otimes}\sfI_{\calH_\text{env}}.
\end{align}
representing the joint evolution of system and environment, where use used the fact that
\[
	\d\langle \sfX^2\rangle = -L^\dagger L\dd\lambda.
\]
...
\begin{remark}
	...
	\]
	i.e., we recover the scenario of Section~\ref{sec:open-quantum} with $\sfB=\sfA+\sfA^\dagger$ as introduced in Section~\ref{sec:quantum-noise}.
\end{remark}

\vspace{-0.8em}
\paragraph{Towards the Lindblad equation} The Lindblad equation is obtained by conjugating the state $\rho_0\otimes \ket{\Upomega}\bra{\Upomega}\in \scrD(\calH_\text{tot})$ with the unitary evolution $U_t$. Setting
\[
	\oba_t\coloneqq U_t(\rho_0\otimes \ket{\Upomega}\bra{\Upomega})U_t^\dagger,
\]
we obtain for $\oba_t$ the evolution
\begin{align*}
	\d\oba_t &= \bigl( \d\sfX(t) + \frI \dd t \bigr) \oba_t + \oba_t\bigl( -\d\sfX(t) + \frI\dd t \bigr) + L^\dagger{\otimes}\sfI_{\calH_\text{env}}\oba_t L{\otimes}\sfI_{\calH_\text{env}}\dd t \\
	&= \bigl(-i[H{\otimes}\sfI_{\calH_\text{env}},\oba_t] - \tfrac{1}{2}[L^\dagger{\otimes}\sfI_{\calH_\text{env}},[L{\otimes}\sfI_{\calH_\text{env}},\oba_t]]\bigr)\d t + [L{\otimes}\dd\sfA^\dagger-L^\dagger{\otimes}\dd\sfA,\oba_t].
\end{align*}
Now, since $\sfI_{\calF_{t]}}{\otimes}\dd\sfA$ commutes with $\oba_t$ and $L{\otimes}\sfI_{\calH_\text{env}}$---recall that $\sfa_t$ is $\calF$-adapted---we have
\begin{align*}
	&\langle\Upomega,[L{\otimes}\dd\sfA^\dagger-L^\dagger{\otimes}\dd\sfA,\oba_t]\Upomega\rangle \\
	&\qquad= -\langle L{\otimes}\dd\sfA^\dagger\Upomega,\oba_t\Upomega\rangle - \langle \Upomega,\oba_t L{\otimes}\dd\sfA^\dagger\Upomega\rangle \\
	&\qquad= -\langle L{\otimes}\sfI_{\calH_\text{env}}\Upomega,\oba_t \sfI_{\calF_{t]}}{\otimes}\dd\sfA\Upomega\rangle - \langle \sfI_{\calF_{t]}}{\otimes}\dd\sfA\Upomega,\oba_t L{\otimes}\sfI_{\calH_\text{env}}\Upomega\rangle = 0.
\end{align*}
Hence, taking the partial trace w.r.t.\ $\Upomega$ and setting $\rho_t\coloneqq \langle \Upomega,\sfa_t\Upomega\rangle$, we obtain
\begin{align*}
	\d\rho_t = -i[H,\rho_t]\dd t  - \tfrac{1}{2}[L^\dagger,[L,\rho_t]]\dd t,
\end{align*}
which is precisely the general Lindblad equation \eqref{eq:lindblad}.

\subsection{Example: 1-qubit fidelity under amplitude damping}
Consider a single qubit with driving Hamiltonian $H = H_\sfh\in\scrO(\calH_\text{sys})$ and noise operator $L = \sqrt{\gamma} \sigma_-=\sqrt{\gamma}\ket{\e_0}\bra{\e_1}\in\scrB(\calH_\text{sys})$, describing spontaneous emission. Recall from the example in Section~\ref{sec:open-quantum} that  $H_\sfh = \ket{\e_-}\bra{\e_-}$, where $\e_- = (\e_0-\e_1)/\sqrt{2}$. 

Under these operators, the QSDE becomes
\[
	\d U_t^\upalpha = \bigl(-iH_\sfh{\otimes}\sfI_{\calH_\text{env}}\dd t + \sqrt{\gamma} \sigma_-{\otimes}  \dd\sfA^\dagger(t) - \sqrt{\gamma} \sigma_-^\dagger{\otimes}  \dd\sfA(t) - \tfrac{\gamma}{2} \sigma_-^\dagger \sigma_- \dd t \bigr) U_t^\upalpha,
\]
with the explicit solution $U_t=\exp(\sfX(t))$, where
\[
	\sfX(t) = -itH_\sfh{\otimes}\sfI_{\calH_\text{env}} +\sqrt{\gamma} \sigma_-{\otimes} \sfA^\dagger(t) - \sqrt{\gamma} \sigma_-^\dagger{\otimes} \sfA(t)\;\;\in\scrA(\calH_\text{tot}).
\]

As in Section~\ref{sec:open-quantum}, we compute the fidelity
\begin{align*}
	\sfF_t = |\langle\psi,U_t^\dagger U_t^\upalpha\psi\rangle|^2 = 
\end{align*}
\[
	[H_\sfh,\sigma_-] = \ket{\e_-}\bra{\e_-}\ket{\e_0}\bra{\e_1} - \ket{\e_0}\bra{\e_1}\ket{\e_-}\bra{\e_-} = \tfrac{1}{\sqrt{2}}\bigl(\ket{\e_-}\bra{\e_1} + \ket{\e_0}\bra{\e_-}\bigr)
\]

\[
	[H_\sfh,\sigma_-^\dagger] = \ket{\e_-}\bra{\e_-}\ket{\e_1}\bra{\e_0} - \ket{\e_1}\bra{\e_0}\ket{\e_-}\bra{\e_-} = -\tfrac{1}{\sqrt{2}}\bigl(\ket{\e_-}\bra{\e_0} + \ket{\e_1}\bra{\e_-}\bigr)
\]
\[
	\sigma_-^\dagger\sigma_- = \ket{\e_1}\bra{\e_0}\ket{\e_0}\bra{\e_1} = \ket{\e_1}\bra{\e_1}
\]
\[
	[H_\sfh,\sigma_-^\dagger\sigma_-] = \ket{\e_-}\bra{\e_-}\ket{\e_1}\bra{\e_1} - \ket{\e_1}\bra{\e_1}\ket{\e_-}\bra{\e_-} = -\tfrac{1}{\sqrt{2}}\bigl(\ket{\e_-}\bra{\e_1} + \ket{\e_1}\bra{\e_-}\bigr)
\]

Consider $H=\ket{\e_1}\bra{\e_1}$ instead. Then
\[
	[H,\sigma_-] = \ket{\e_1}\bra{\e_1}\ket{\e_0}\bra{\e_1} - \ket{\e_0}\bra{\e_1}\ket{\e_1}\bra{\e_1} = -\ket{\e_0}\bra{\e_1} = -\sigma_-
\]
\[
	[H,\sigma_-^\dagger] = \ket{\e_1}\bra{\e_1}\ket{\e_1}\bra{\e_0} - \ket{\e_1}\bra{\e_0}\ket{\e_1}\bra{\e_1} = \ket{\e_1}\bra{\e_0} =\sigma_-^\dagger
\]
\[
	[H{\otimes}\sfI_{\calH_\text{env}},\sigma_-{\otimes}\sfA^\dagger -\sigma_-^\dagger{\otimes}\sfA] = [H,\sigma_-]{\otimes}  \sfA^\dagger - [H,\sigma_-^\dagger]{\otimes}\sfA = -\sigma_-{\otimes}  \dd\sfA^\dagger - \sigma_-^\dagger{\otimes}  \dd\sfA
\]
\[
	[H,[H,\sigma_-]] = -\ket{\e_1}\bra{\e_1}\ket{\e_0}\bra{\e_1} + \ket{\e_0}\bra{\e_1}\ket{\e_1}\bra{\e_1} = \ket{\e_0}\bra{\e_1} = -[H,\sigma_-]
\]
\[
	[H,[H,\sigma_-^\dagger]] = \ket{\e_1}\bra{\e_1}\ket{\e_1}\bra{\e_0} - \ket{\e_1}\bra{\e_0}\ket{\e_1}\bra{\e_1} = [H,\sigma_-^\dagger]
\]
...
\begin{example}
	...
	\[
		a^-= \frac{1}{\sqrt{2}}\bigl( Q + iP\bigr),\qquad a^+ = \frac{1}{\sqrt{2}}\bigl(Q - iP\bigr),\qquad N\coloneqq a^+a^-
	\]
